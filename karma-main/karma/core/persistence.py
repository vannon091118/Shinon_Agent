...
class SQLiteConnectionManager:
    ...
    def _migrate_v4_to_v5(self, conn: sqlite3.Connection) -> None:
        """Add hash chain columns to events table for audit trail verification."""
        cols = conn.execute("PRAGMA table_info(events)").fetchall()
        col_names = [c[1] for c in cols]
        if "event_hash" not in col_names:
            conn.execute("ALTER TABLE events ADD COLUMN event_hash TEXT")
        if "prev_event_hash" not in col_names:
...
class PersistenceLayer:
    ...
    def emit_event(self, event_type: str, project: Optional[str], payload: Dict, correlation_id: Optional[str] = None) -> None:
        ...
        with self.transaction() as conn:
            # Get last event hash INSIDE transaction to prevent race conditions
            prev_hash = "genesis"
            try:
                prev_hash_row = conn.execute(
                    "SELECT event_hash FROM events WHERE project = ? ORDER BY id DESC LIMIT 1",
                    (project,)
                ).fetchone()
                if prev_hash_row and prev_hash_row["event_hash"]:
                    prev_hash = prev_hash_row["event_hash"]
            except Exception:
                pass  # Column might not exist yet (pre-v5 DB)

            # Compute hash: SHA-256(prev_hash + event_type + payload + timestamp + correlation_id)
            hash_input = f"{prev_hash}|{event_type}|{payload_json}|{timestamp}|{correlation_id or ''}"
            event_hash = hashlib.sha256(hash_input.encode()).hexdigest()[:16]

            # Try v5 insert first, fall back to v4 for pre-v5 DBs
            try:
                conn.execute("""
                    INSERT INTO events (event_type, project, payload, timestamp, correlation_id, event_hash, prev_event_hash)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (event_type, project, payload_json, timestamp, correlation_id, event_hash, prev_hash))
            except Exception:
                # Pre-v5 DB — insert without hash columns
                conn.execute("""
                    INSERT INTO events (event_type, project, payload, timestamp, correlation_id)
                    VALUES (?, ?, ?, ?, ?)
                """, (event_type, project, payload_json, timestamp, correlation_id))
    ...
    def get_events_with_hash_chain(
        self,
        project: Optional[str] = None,
        limit: int = 100
    ) -> List[Dict]:
        """Get events with hash chain columns for audit verification."""
        # Try v5 columns first; fall back to v4 on pre-v5 DBs
        try:
            query = "SELECT id, event_type, project, payload, timestamp, correlation_id, event_hash, prev_event_hash FROM events WHERE 1=1"
            params = []
            if project:
                query += " AND project = ?"
                params.append(project)
            ...
            # Pre-v5 DB — hash columns don't exist yet
            query = "SELECT id, event_type, project, payload, timestamp, correlation_id FROM events WHERE 1=1"
            params = []
            if project:
                query += " AND project = ?"
                params.append(project)
            ...
            # Add missing keys so callers get consistent shapes
            for r in results:
                r.setdefault("event_hash", None)
                r.setdefault("prev_event_hash", None)
            return results

    def get_last_event_hash(self, project: str) -> Optional[str]:
        """Get the hash of the last event in the chain for a project."""
        try:
            row = self.fetchone(
                "SELECT event_hash FROM events WHERE project = ? ORDER BY id DESC LIMIT 1",
                (project,)
