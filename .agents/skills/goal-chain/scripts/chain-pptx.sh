#!/bin/bash
# chain-pptx.sh — STACK: MEMORY
# Auto-generated wrap of claude-tools/pptx/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-pptx.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: pptx"
emit_user_input_start "chain-pptx.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: claude-tools/pptx/SKILL.md

## Aufgabe
PowerPoint-Operationen. .pptx lesen/schreiben via python-pptx. Self-Check: Slides + Layouts erhalten.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/claude-tools/pptx/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/claude-tools/pptx/SKILL.md`) NICHT veraendern.
