#!/bin/bash
# chain-screenshot-tools.sh — STACK: MEMORY
# Auto-generated wrap of media/screenshot-tools/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-screenshot-tools.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: screenshot-tools"
emit_user_input_start "chain-screenshot-tools.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: media/screenshot-tools/SKILL.md

## Aufgabe
Screenshot-Capture + Annotation. ImageMagick/Pillow. Self-Check: Output-Format + DPI.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/media/screenshot-tools/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/media/screenshot-tools/SKILL.md`) NICHT veraendern.
