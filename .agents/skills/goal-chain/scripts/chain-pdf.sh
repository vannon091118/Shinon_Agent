#!/bin/bash
# chain-pdf.sh — STACK: MEMORY
# Auto-generated wrap of claude-tools/pdf/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-pdf.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: pdf"
emit_user_input_start "chain-pdf.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: claude-tools/pdf/SKILL.md

## Aufgabe
PDF-Operationen. Lesen/Extrahieren/Verbinden/Teilen/OCR via pypdf + pdfplumber. Self-Check: Output-Validierung (Pages, Text-Länge).

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/claude-tools/pdf/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/claude-tools/pdf/SKILL.md`) NICHT veraendern.
