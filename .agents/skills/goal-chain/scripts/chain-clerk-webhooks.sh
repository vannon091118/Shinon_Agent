#!/bin/bash
# chain-clerk-webhooks.sh — STACK: AUTONOM
# Auto-generated wrap of agents/clerk-webhooks/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-clerk-webhooks.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "AUTONOM: clerk-webhooks"
emit_user_input_start "chain-clerk-webhooks.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: agents/clerk-webhooks/SKILL.md

## Aufgabe
Webhook-Integration mit Clerk (Auth-Provider). Analysiere Clerk-Webhook-Events, baue sichere Endpoints mit Signature-Verification. Self-Check: Idempotenz + Replay-Schutz.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/agents/clerk-webhooks/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/agents/clerk-webhooks/SKILL.md`) NICHT veraendern.
