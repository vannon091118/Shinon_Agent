#!/bin/bash
# chain-evil-twin-protocol.sh — STACK: GOVERNANCE
# Auto-generated wrap of evil-twin-protocol/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-evil-twin-protocol.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "GOVERNANCE: evil-twin-protocol"
emit_user_input_start "chain-evil-twin-protocol.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: evil-twin-protocol/SKILL.md

## Aufgabe
Standalone Evil-Twin-Engine. Spiegel-Thinker mit identischer Datenlage. FUNDAMENTAL widersprechen — nicht an Kleinigkeiten aufhalten. Synthese erzeugen.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/evil-twin-protocol/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/evil-twin-protocol/SKILL.md`) NICHT veraendern.
