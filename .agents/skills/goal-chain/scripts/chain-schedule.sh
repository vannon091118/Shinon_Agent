#!/bin/bash
# chain-schedule.sh — STACK: MEMORY
# Auto-generated wrap of claude-tools/schedule/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-schedule.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: schedule"
emit_user_input_start "chain-schedule.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: claude-tools/schedule/SKILL.md

## Aufgabe
Aufgaben-Planung und Zeitfenster. Baue Schedule-Tabelle mit Moeglichkeits-/Deadline-Tracking. Self-Check: keine Konflikte + Realistic-Buffer.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/claude-tools/schedule/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/claude-tools/schedule/SKILL.md`) NICHT veraendern.
