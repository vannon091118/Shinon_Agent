#!/bin/bash
# chain-pdf-tools.sh — STACK: MEMORY
# Auto-generated wrap of documents/pdf-tools/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-pdf-tools.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: pdf-tools"
emit_user_input_start "chain-pdf-tools.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: documents/pdf-tools/SKILL.md

## Aufgabe
PDF Convenience-Wrapper. Higher-Level Operationen als 'pdf'-Skill. Self-Check: Kommandozeilen-Pattern idempotent.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/documents/pdf-tools/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/documents/pdf-tools/SKILL.md`) NICHT veraendern.
