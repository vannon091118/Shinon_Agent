#!/bin/bash
# chain-document-tools.sh — STACK: MEMORY
# Auto-generated wrap of documents/document-tools/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-document-tools.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: document-tools"
emit_user_input_start "chain-document-tools.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: documents/document-tools/SKILL.md

## Aufgabe
Generisches Dokument-Tool. Markdown/MD/RST lesen+schreiben. Self-Check: Encoding + Line-Endings korrekt.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/documents/document-tools/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/documents/document-tools/SKILL.md`) NICHT veraendern.
