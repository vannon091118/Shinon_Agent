#!/bin/bash
# chain-spreadsheet-tools.sh — STACK: MEMORY
# Auto-generated wrap of documents/spreadsheet-tools/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-spreadsheet-tools.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: spreadsheet-tools"
emit_user_input_start "chain-spreadsheet-tools.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: documents/spreadsheet-tools/SKILL.md

## Aufgabe
Spreadsheet-Tool. CSV/TSV/XLSX lesen+schreiben. Self-Check: Schema-Validierung.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/documents/spreadsheet-tools/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/documents/spreadsheet-tools/SKILL.md`) NICHT veraendern.
