#!/bin/bash
# chain-sub-agent-prompts.sh — STACK: AUTONOM
# Auto-generated wrap of agents/sub-agent-prompts/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-sub-agent-prompts.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "AUTONOM: sub-agent-prompts"
emit_user_input_start "chain-sub-agent-prompts.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: agents/sub-agent-prompts/SKILL.md

## Aufgabe
Sub-Agent-Prompt-Library. Erstelle robuste Prompt-Templates (Code-Reviewer, Playwright-Debugger, Repo-Hygiene-Mapper). Self-Check: Rolle+Ziel+Pflichtchecks+Ausgabeformat.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/agents/sub-agent-prompts/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/agents/sub-agent-prompts/SKILL.md`) NICHT veraendern.
