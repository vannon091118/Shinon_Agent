#!/bin/bash
# chain-performance.sh — STACK: KREATIV
# Auto-generated wrap of design/performance/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-performance.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "KREATIV: performance"
emit_user_input_start "chain-performance.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: design/performance/SKILL.md

## Aufgabe
Web-Performance-Optimierung. LCP/FID/CLS + Critical-Path. Self-Check: Lighthouse-Score >= 90.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/design/performance/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/design/performance/SKILL.md`) NICHT veraendern.
