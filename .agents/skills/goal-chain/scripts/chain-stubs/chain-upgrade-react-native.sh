#!/bin/bash
# chain-upgrade-react-native.sh — STACK: LOGISCH
# Auto-generated wrap of development/upgrade-react-native/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-upgrade-react-native.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "LOGISCH: upgrade-react-native"
emit_user_input_start "chain-upgrade-react-native.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: development/upgrade-react-native/SKILL.md

## Aufgabe
React-Native Upgrade-Pfad. Breaking-Changes dokumentieren + Migration-Codmods. Self-Check: alle deprecated APIs ersetzt.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/development/upgrade-react-native/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/development/upgrade-react-native/SKILL.md`) NICHT veraendern.
