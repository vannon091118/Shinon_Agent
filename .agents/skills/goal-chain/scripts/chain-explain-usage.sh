#!/bin/bash
# chain-explain-usage.sh — STACK: MEMORY
# Auto-generated wrap of claude-tools/explain-usage/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-explain-usage.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: explain-usage"
emit_user_input_start "chain-explain-usage.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: claude-tools/explain-usage/SKILL.md

## Aufgabe
Erklaere Tool-Nutzung detailliert. Wann nutzen, wie nutzen, Fallstricke. Self-Check: 1 Beispiel + 1 Anti-Pattern.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/claude-tools/explain-usage/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/claude-tools/explain-usage/SKILL.md`) NICHT veraendern.
