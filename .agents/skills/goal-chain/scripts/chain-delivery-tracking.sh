#!/bin/bash
# chain-delivery-tracking.sh — STACK: AUTONOM
# Auto-generated wrap of agents/delivery-tracking/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-delivery-tracking.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "AUTONOM: delivery-tracking"
emit_user_input_start "chain-delivery-tracking.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: agents/delivery-tracking/SKILL.md

## Aufgabe
Delivery-Tracking-Pipeline. State-Machine fuer Sendungs-Status (pending→in_transit→delivered). Self-Check: Eventual Consistency + Audit-Trail.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/agents/delivery-tracking/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/agents/delivery-tracking/SKILL.md`) NICHT veraendern.
