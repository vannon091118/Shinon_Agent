#!/bin/bash
# chain-morning.sh — STACK: MEMORY
# Auto-generated wrap of claude-tools/morning/SKILL.md
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/tid-helpers.sh"

RUN_ID="${1:?Usage: bash chain-morning.sh RUN_ID TID}"
TID="${2:?}"
...
agent_header "$TID" "MEMORY: morning"
emit_user_input_start "chain-morning.sh"

cat <<INSTRUCTION

## Goal: $GOAL
## Skill: $SKILL
## Source: claude-tools/morning/SKILL.md

## Aufgabe
Morning-Routine / Tagesplaner. Lade ToDos, Calendar, priorisiere. Self-Check: 3 wichtigste Tasks identifiziert.

## Domain-Kontext
- Original-Skill liegt unter: `.agents/skills/claude-tools/morning/SKILL.md`
...
## Agent-Regeln
1. Source-Skills (`.agents/skills/claude-tools/morning/SKILL.md`) NICHT veraendern.
