---
skill: virtual-agent-ios
state: idle
last_activation: 2026-08-11T23:01:09Z
activation_count: 2
tags: [catalog]
output_path: ".agents/skills/communication-apis/zoom/virtual-agent/ios/SKILL.md"
---

# ⏸️  virtual-agent-ios · IDLE

> **Token-saving artifact.** Dieser ~20-Zeilen-Snapshot ersetzt das vollständige
> SKILL.md (~200-400 Zeilen) bei Re-Activation. Re-Aktivierungen lesen NUR
> diesen Snapshot, sofern keine erweiterte Funktionalität benötigt wird.

## Aktueller Status
"Zoom Virtual Agent iOS integration via WKWebView. Use for Swift/Objective-C script injection, message handlers, support_handoff relay, and URL routing policies."

## Pfad zum Output
.agents/skills/communication-apis/zoom/virtual-agent/ios/SKILL.md

## Re-Aktivierung (schnell)
```bash
bash .agents/skills/live-context.sh virtual-agent-ios   # holt diesen Snapshot
# Falls mehr Details nötig:  cat .agents/skills/virtual-agent-ios/SKILL.md
```

